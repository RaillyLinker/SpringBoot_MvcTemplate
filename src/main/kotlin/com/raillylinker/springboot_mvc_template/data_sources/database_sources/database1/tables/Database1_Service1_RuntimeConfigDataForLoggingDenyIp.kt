...
@Entity
@Table(
    name = "runtime_config_data_for_logging_deny_ip",
    catalog = "service1",
    uniqueConstraints = [
        UniqueConstraint(columnNames = ["runtime_config_data_uid", "ip_string"])
    ]
)
@Comment("Service1 의 런타임 변경 가능 설정 정보 중 로깅 대상에서 제외할 IP")
class Database1_Service1_RuntimeConfigDataForLoggingDenyIp(
    @ManyToOne
    ...
    @Comment("로깅 대상에서 제외할 IP (ex : 127.0.0.1)")
    var ipString: String,
