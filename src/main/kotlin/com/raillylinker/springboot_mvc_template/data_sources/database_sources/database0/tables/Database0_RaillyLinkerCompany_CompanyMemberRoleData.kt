...
@Entity
@Table(
    name = "company_member_role_data",
    catalog = "railly_linker_company",
    uniqueConstraints = [
        UniqueConstraint(columnNames = ["company_member_uid", "role"])
    ]
)
@Comment("회원 권한 정보 테이블")
class Database0_RaillyLinkerCompany_CompanyMemberRoleData(
    @ManyToOne
    @JoinColumn(name = "company_member_uid", nullable = false)
    @Comment("멤버 고유번호(railly_linker_company.company_member_data.uid)")
    var companyMemberData: Database0_RaillyLinkerCompany_CompanyMemberData,
